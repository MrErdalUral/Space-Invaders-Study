(function(window){
	var STEP = 1, //movement speed of the player higher values will cause choppy movement
		...
		ALIEN_STATE_COUNTER_TRESHOLD=200,
		MISSILE_STATE_COUNTER_TRESHOLD=50;//determines how many steps aliens wait before taking a step.
	...
	gameLogic.prototype.StartLoop = function(){
		var elements = this.Elements,
			...
			drawStateCounter = 0,
			missileStateCounter = 0;
		setInterval(function(){
			
			if(elements.Missile && 
			elements.Alien && 
			elements.Alien.X - elements.Missile.X <= elements.Missile.width && 
			elements.Alien.X - elements.Missile.X >= -elements.Alien.width && 
			elements.Missile.Y - elements.Alien.Y < elements.Alien.height && 
			elements.Missile.Y - elements.Alien.Y > -elements.Missile.height){
				delete elements.Alien;
			...
			if(elements.Missile){
				...
				if(missileStateCounter >= MISSILE_STATE_COUNTER_TRESHOLD){
					missileStateCounter = 0;
					elements.Missile.DrawState = (elements.Missile.DrawState+1)%2;
				}
				missileStateCounter+=1;
			}
